...
func (d *Document) parseIndirect(start int) (*model.IndirectObject, error) {
	...
	headerEnd := pos
	body, n, err := syntax.ParseObjectWithLimits(d.data[pos:], 1, int64(pos), d.Options.Limits)
	if err != nil {
		return nil, err
	}
	pos += n
	...
	if word == "stream" {
		dict, ok := body.Value.(model.Dictionary)
		if !ok {
		...
		pos += 9
		encoded := model.Span{Source: 1, Start: int64(dataStart), End: int64(dataEnd)}
		endKeyword := model.Span{Source: 1, Start: int64(endStart), End: int64(pos)}
		body.Value = model.Stream{Dictionary: dict, DictionarySpan: body.Span, StartKeyword: model.Span{Source: 1, Start: int64(keywordStart), End: int64(keywordEnd)}, DataStart: model.Position{Source: 1, Offset: int64(dataStart)}, Encoded: &encoded, EndKeyword: &endKeyword, Boundary: model.StreamFromLength}
	...
	endobj := model.Span{Source: 1, Start: int64(keywordStart), End: int64(pos)}
	whole := model.Span{Source: 1, Start: int64(begin), End: int64(pos)}
	obj := &model.IndirectObject{ID: model.ObjectID{Number: uint32(number), Generation: uint16(generation)}, Body: body, Origin: model.FileObjectOrigin{Whole: whole, Header: model.Span{Source: 1, Start: int64(begin), End: int64(headerEnd)}, EndObj: &endobj}}
	...
	d.markRegion(model.RegionIndirectObject, whole)
	return obj, nil
}
...
func (d *Document) loadCompressed(id model.ObjectID, entry model.CompressedEntry) (*model.IndirectObject, error) {
	...
	for i := int64(0); i < n; i++ {
		...
		pairs = append(pairs, pair{number: uint32(num), offset: int64(off), span: model.Span{Source: source.ID, Start: int64(begin), End: int64(pos)}})
	}
	...
	body, consumed, err := syntax.ParseObjectWithLimits(data[start:end], source.ID, start, d.Options.Limits)
	if err != nil {
		return nil, err
	}
	...
	origin, ok := container.Origin.(model.FileObjectOrigin)
	if !ok {
		return nil, errors.New("object stream must be a file object")
	}
	obj := &model.IndirectObject{ID: id, Body: body, Origin: model.ObjectStreamOrigin{Container: container.ID, ContainerSpan: origin.Whole, Index: entry.Index, HeaderPair: p.span}}
	if len(d.Structure.Objects) >= d.Options.Limits.MaxObjects {
		return nil, errors.New("parsed object limit exceeded")
	}
	d.Structure.Objects = append(d.Structure.Objects, *obj)
