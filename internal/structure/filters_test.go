...
func TestStreamDecodeBudgetAndWrongLength(t *testing.T) {
	...
	d := parseTestPDF(t, data)
	if _, err := d.Load(model.ObjectID{Number: 2}); err == nil {
	...
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: model.Limits{MaxDecodedBytes: 64}})
	if err != nil {
		t.Fatal(err)
	}
	obj, err := d.Resolve(model.Reference{ID: model.ObjectID{Number: 2}})
	...
	if _, err = d.DecodeStream(obj.Value.(model.Stream)); err == nil {
