...
func (d *Document) readHeaderAndXRefs() error {
	...
	d.Structure.Header = &model.Header{Span: model.Span{Source: 1, Start: int64(header), End: int64(header + 8)}, Version: model.Version{Major: version[0] - '0', Minor: version[2] - '0'}}
	d.markRegion(model.RegionHeader, d.Structure.Header.Span)
	if header != 0 {
	...
	tail := model.FileTail{StartXRef: model.Span{Source: 1, Start: int64(start), End: int64(start + 9)}, Offset: int64(offset), OffsetRaw: model.Span{Source: 1, Start: int64(raw), End: int64(rawEnd)}, EOFMarker: model.Span{Source: 1, Start: int64(pos), End: int64(pos + 5)}}
	d.Structure.Tails = append(d.Structure.Tails, tail)
	d.markRegion(model.RegionFileTail, model.Span{Source: 1, Start: int64(start), End: int64(pos + 5)})
...
func (d *Document) readXRefChain(offset int64, seen map[int64]bool) error {
	...
	section.ID = model.SectionID(len(d.Structure.XRefs) + 1)
	...
	if section.XRefStm != nil {
		...
		if hybrid.Form != model.XRefStream {
			return errors.New("XRefStm does not refer to xref stream")
		}
		hybrid.ID = model.SectionID(len(d.Structure.XRefs) + 1)
...
func (d *Document) readXRefTable(start int) (model.XRefSection, error) {
	...
	for {
		...
		if word == "trailer" {
			obj, n, err := syntax.ParseObjectWithLimits(d.data[pos:], 1, int64(pos), d.Options.Limits)
			if err != nil {
				return section, err
			}
			pos += n
			section.Trailer = obj
			section.Span = model.Span{Source: 1, Start: int64(start), End: int64(pos)}
			d.markRegion(model.RegionXRef, model.Span{Source: 1, Start: int64(start), End: int64(begin)})
			d.markRegion(model.RegionTrailer, model.Span{Source: 1, Start: int64(begin), End: int64(pos)})
		...
		header := model.Span{Source: 1, Start: int64(begin), End: int64(pos)}
		r := model.XRefRange{First: uint32(first), Count: uint32(count), Header: &header}
		for i := uint64(0); i < count; i++ {
			...
			record := model.XRefRecord{Number: uint32(first + i), Span: model.Span{Source: 1, Start: int64(recordStart), End: int64(pos)}}
			switch flag {
			case "n":
				record.Entry = model.InUseEntry{Offset: int64(off), Generation: uint16(gen)}
			case "f":
				if off > 1<<32-1 {
					return section, errors.New("free object number overflows uint32")
				}
				record.Entry = model.FreeEntry{NextFree: uint32(off), Generation: uint16(gen)}
			default:
...
func (d *Document) readXRefStream(start int) (model.XRefSection, error) {
	...
	stream, ok := obj.Body.Value.(model.Stream)
	if !ok {
		return section, errors.New("xref target is not a stream")
	}
	kind, err := stream.Dictionary.Get("Type")
	if err != nil || kind.Value != model.Name("XRef") {
		return section, errors.New("xref stream lacks /Type /XRef")
	}
	section.StreamID = &obj.ID
	section.Span = obj.Origin.(model.FileObjectOrigin).Whole
	section.Trailer = model.Object{Span: stream.DictionarySpan, Value: stream.Dictionary}
	source, err := d.DecodeStream(stream)
	if err != nil {
		return section, err
	}
	data, err := d.Bytes(model.Span{Source: source.ID, End: source.Size})
	if err != nil {
		return section, err
	}
	...
	warr, ok := wobj.Value.(model.Array)
	...
	for i := 0; i < len(indices); i += 2 {
		...
		for j := int64(0); j < count; j++ {
			...
			record := model.XRefRecord{Number: uint32(first + j), Span: model.Span{Source: source.ID, Start: int64(begin), End: int64(pos)}}
			switch fields[0] {
			case 0:
				if fields[1] > 1<<32-1 || fields[2] > 65535 {
					return section, errors.New("xref free entry overflow")
				}
				record.Entry = model.FreeEntry{NextFree: uint32(fields[1]), Generation: uint16(fields[2])}
			case 1:
				if fields[1] > uint64(len(d.data)) || fields[2] > 65535 {
					return section, errors.New("xref in-use entry out of range")
				}
				record.Entry = model.InUseEntry{Offset: int64(fields[1]), Generation: uint16(fields[2])}
			case 2:
