...
func TestLexPreservesTriviaAndSourceSpans(t *testing.T) {
	...
	want := []struct {
		...
		{model.TokenWhitespace, " \x00"}, {model.TokenComment, "%hi"}, {model.TokenWhitespace, "\r\n"},
		{model.TokenName, "/A#20B"}, {model.TokenWhitespace, " "}, {model.TokenInteger, "01"},
		{model.TokenWhitespace, " "}, {model.TokenReal, "+.5"}, {model.TokenWhitespace, " "},
		{model.TokenLiteralString, "(x)"}, {model.TokenWhitespace, " "}, {model.TokenHexString, "<F>"},
		{model.TokenWhitespace, " "}, {model.TokenArrayOpen, "["}, {model.TokenArrayClose, "]"},
		{model.TokenWhitespace, " "}, {model.TokenDictOpen, "<<"}, {model.TokenDictClose, ">>"},
		{model.TokenWhitespace, " "}, {model.TokenKeyword, "q"}, {model.TokenWhitespace, " "},
		{model.TokenKeyword, "true"}, {model.TokenEOF, ""},
	}
...
func TestParseObjectPrimitiveValues(t *testing.T) {
	tests := []struct {
		...
		{"+00012", model.Integer("+00012")}, {"-0", model.Integer("-0")}, {".50", model.Real(".50")},
		{"+1.", model.Real("+1.")}, {"-.0020", model.Real("-.0020")}, {"true", model.Boolean(true)},
		{"false", model.Boolean(false)}, {"null", model.Null{}}, {"/A#20B#ff", model.Name("A B\xff")},
		{"/", model.Name("")}, {"()", model.PDFString{Form: model.StringLiteral, Bytes: []byte{}}},
		{"(a(b)c)", model.PDFString{Form: model.StringLiteral, Bytes: []byte("a(b)c")}},
		{`(a\101\12\1\777\(\)\\\n\r\t\b\f\z)`, model.PDFString{Form: model.StringLiteral, Bytes: []byte("aA\n\x01\xff()\\\n\r\t\b\fz")}},
		{"(a\rb\r\nc\nd)", model.PDFString{Form: model.StringLiteral, Bytes: []byte("a\nb\nc\nd")}},
		{"(a\\\r\nb\\\rc\\\nd)", model.PDFString{Form: model.StringLiteral, Bytes: []byte("abcd")}},
		{"<A b C\x00>", model.PDFString{Form: model.StringHex, Bytes: []byte{0xab, 0xc0}}},
		{"<>", model.PDFString{Form: model.StringHex, Bytes: []byte{}}},
		{"12 0 R", model.Reference{ID: model.ObjectID{Number: 12}}},
		{"4294967295 65535 R", model.Reference{ID: model.ObjectID{Number: 4294967295, Generation: 65535}}},
		{"12 % ref\r\n 34 R", model.Reference{ID: model.ObjectID{Number: 12, Generation: 34}}},
	}
	for _, test := range tests {
		t.Run(test.input, func(t *testing.T) {
			...
			if n != len(test.input) || got.Span != (model.Span{Source: 2, Start: 100, End: 100 + int64(len(test.input))}) {
...
func TestParseObjectPreservesDictionaryEntriesAndNestedSpans(t *testing.T) {
	...
	want := model.Object{Span: model.Span{Source: 4, Start: 102, End: 141}, Value: model.Dictionary{Entries: []model.DictionaryEntry{
		{Key: "A", KeySpan: model.Span{Source: 4, Start: 105, End: 107}, Value: model.Object{Span: model.Span{Source: 4, Start: 108, End: 115}, Value: model.PDFString{Form: model.StringLiteral, Bytes: []byte("aA")}}},
		{Key: "A", KeySpan: model.Span{Source: 4, Start: 116, End: 118}, Value: model.Object{Span: model.Span{Source: 4, Start: 119, End: 138}, Value: model.Array{Items: []model.Object{
			{Span: model.Span{Source: 4, Start: 120, End: 126}, Value: model.Reference{ID: model.ObjectID{Number: 12, Generation: 0}}},
			{Span: model.Span{Source: 4, Start: 127, End: 132}, Value: model.Boolean(false)},
			{Span: model.Span{Source: 4, Start: 133, End: 137}, Value: model.Null{}},
		}}}},
	}}}
...
func TestParseObjectStopsBeforeTrailingTriviaAndOtherObjects(t *testing.T) {
	for _, input := range []string{" 12 34", " 12 q", " 12 (unterminated", " 12 % trailing", " 12 <invalid", " 12"} {
		...
		if err != nil || n != 3 || got.Value != model.Integer("12") || got.Span != (model.Span{Source: 1, Start: 10, End: 12}) {
...
func TestSyntaxRejectsInvalidOffsetsAndEnforcesLimits(t *testing.T) {
	...
	if _, _, err := ParseObjectWithLimits([]byte("[[[]]]"), 1, 0, model.Limits{MaxDepth: 2}); err == nil {
		t.Fatal("ignored custom depth limit")
	}
	if _, _, err := ParseObjectWithLimits([]byte("(12345)"), 1, 0, model.Limits{MaxTokenBytes: 6}); err == nil {
		t.Fatal("ignored custom token byte limit")
	}
	if _, _, err := ParseObjectWithLimits([]byte("true"), 1, 0, model.Limits{MaxDepth: -1}); err == nil {
		t.Fatal("accepted negative depth limit")
	}
	if _, _, err := ParseObjectWithLimits([]byte("true"), 1, 0, model.Limits{MaxTokenBytes: -1}); err == nil {
		t.Fatal("accepted negative token limit")
	}
	if _, _, err := ParseObjectWithLimits([]byte("true"), 1, 0, model.Limits{MaxDepth: 1_000_000}); err == nil {
...
func FuzzLex(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if err == nil && (len(tokens) == 0 || tokens[len(tokens)-1].Kind != model.TokenEOF || next != 3+int64(len(data))) {
