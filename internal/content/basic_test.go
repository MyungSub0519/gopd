...
func TestBasicContentOrderAndSharedResources(t *testing.T) {
	...
	if first.Matrix != (model.Matrix{2, 0, 0, 3, 50, 110}) {
	...
	if graphic.Page != 0 || !graphic.Stroke || graphic.Fill || graphic.Style.LineWidth != 2 || graphic.Segments[1].Points[0] != (model.Point{X: 30, Y: 50}) {
		t.Fatalf("graphic placement or style changed: %+v", graphic)
	}
	if detail.Images[0].Resource != detail.Images[1].Resource || detail.Images[0].Matrix != (model.Matrix{2, 0, 0, 3, 10, 20}) {
...
func TestBasicDiagnosticsKeepPageForRepeatedForms(t *testing.T) {
	...
	for i, issue := range detail.Diagnostics {
		if issue.Severity != model.SeverityWarning || issue.Code != "unsupported-content-effect" || detail.Pages[i].Complete || !p.Graphics[i][0].Style.Clipped || p.Graphics[i][0].Style.Complete {
...
func TestBasicUserPDF(t *testing.T) {
	// The fixture lives at the repository root, two levels above this package.
	...
	if len(p.Texts) != 3 || len(p.Graphics) != 3 || len(p.Details().Images) != 1 || len(p.Details().Diagnostics) != 8 {
