...
func TestContentMixedOrderUnicodeAndRepeatedImage(t *testing.T) {
	...
	if p.Images[0].Matrix != (model.Matrix{2, 0, 0, 3, 10, 20}) || p.Images[1].Matrix != model.IdentityMatrix() {
...
func TestContentHonorsGlyphAndOperandDepthLimits(t *testing.T) {
	for _, content := range []string{`BT /F 10 Tf (` + strings.Repeat("A", 101) + `) Tj ET`, strings.Repeat("[", 20) + strings.Repeat("]", 20) + ` DP`} {
		...
		d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: model.Limits{MaxObjects: 100, MaxDepth: 8}})
		if err != nil {
			t.Fatal(err)
		}
...
func TestContentBoundsRepeatedEmptyPageTreeNodes(t *testing.T) {
	...
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: model.Limits{MaxObjects: 20}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err = BuildPDF(d); err == nil || !strings.Contains(err.Error(), "limit") {
...
func TestContentBoundsExpandedFontMaps(t *testing.T) {
	...
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: model.Limits{MaxObjects: 100}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err = BuildPDF(d); err == nil || !strings.Contains(err.Error(), "limit") {
...
func TestContentBoundsExpandedUnicodeText(t *testing.T) {
	...
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: model.Limits{MaxDecodedBytes: 1024}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err = BuildPDF(d); err == nil || !strings.Contains(err.Error(), "limit") {
