...
func reviewDecodeWithLimit(t *testing.T, filter string, payload []byte, limit int64) ([]byte, error) {
	...
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: Limits{MaxDecodedBytes: limit}})
	if err != nil {
		return nil, err
	}
	o, err := d.Resolve(Reference{ID: ObjectID{Number: 2}})
	if err != nil {
		return nil, err
	}
	s, err := d.DecodeStream(o.Value.(Stream))
	if err != nil {
		return nil, err
	}
	return d.Bytes(Span{Source: s.ID, End: s.Size})
}
...
func TestReviewReaderRejectsFalseStreamBoundary(t *testing.T) {
	...
	o, err := d.Load(ObjectID{Number: 2})
	if err == nil {
		s := o.Body.Value.(Stream)
...
func TestReviewReaderEmptyXRefRangesNeedBudget(t *testing.T) {
	data := []byte("%PDF-1.7\nxref\n" + strings.Repeat("0 0\n", 10000) + "trailer\n<< /Size 0 >>\nstartxref\n9\n%%EOF\n")
	d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{Limits: Limits{MaxObjects: 2}})
	if err == nil {
...
func TestReviewReaderHistoricalXRefRecordsNeedBudget(t *testing.T) {
	...
	d, err := Parse(bytes.NewReader(file.Bytes()), int64(file.Len()), ReadOptions{Limits: Limits{MaxObjects: 8}})
	if err == nil {
		total := 0
