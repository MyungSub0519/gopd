...
이 파일은 콘텐츠 정보와 정밀 분석 정보를 함께 담은 기존 상세 결과입니다. 현재 `pdf.ParsePDF()`의 기본 객체와 기본 JSON은 [기본 PDF API](basic-pdf.md)에서 설명합니다. 기존 상세 JSON 파일은 변경하지 않았습니다. 현재 CLI의 `-json` 옵션은 개수 요약만 출력하므로 두 객체의 전체 JSON과는 출력 범위가 다릅니다. 콘텐츠 종류나 페이지를 선택해 파싱하는 모드는 아직 제공하지 않습니다.
