...
이 문서는 상세 콘텐츠와 파일 구조를 JSON으로 저장할 때 사용할 수 있는 `gopd-parsed-v1` 형식의 키 구조와 참조 관계를 설명합니다. 특정 입력 파일의 파싱 결과를 기록한 문서는 아닙니다.

이 형식은 상세 결과를 위한 별도 직렬화 규칙이며 라이브러리의 기본 JSON 출력 계약은 아닙니다. 현재 `gopd.ParsePDF()`의 기본 객체와 기본 JSON은 [기본 PDF API](basic-pdf.md)에서 설명합니다. 현재 CLI의 `-json` 옵션은 개수 요약만 출력하므로 두 객체의 전체 JSON과는 출력 범위가 다릅니다. 콘텐츠 종류나 페이지를 선택해 파싱하는 모드는 아직 제공하지 않습니다.
...
현재 Go의 `DetailedPDF`가 상세 콘텐츠 모델에 해당합니다. 이 저장 형식에서는 실행 중 사용하는 `Document`를 제외하고 `Input`, `Summary`, `Sources`, `Serialization` 등의 설명 정보를 별도로 구성합니다. 현재 기본 `PDF`는 페이지별 이중 배열 `Texts`와 `Graphics`만 공개하는 별도 타입입니다.
...
주요 `Type`과 `Value`의 관계는 다음과 같습니다.
...
`Boundary` 타입에는 `0=Unresolved`, `1=FromLength`, `2=Recovered`가 정의되어 있습니다. 현재 구현은 길이를 기준으로 경계를 확인하며 손상 경계 자동 복구를 제공하지 않습니다.
...
파일에 직접 저장되는 객체와 사용하지 않는 슬롯은 다음 `Entry.Type`으로 표현합니다.
...
Go 모델은 object stream 안의 압축 객체를 위한 `CompressedEntry`도 정의합니다. 이 경우 `StreamNumber`와 `Index`로 컨테이너 객체 번호와 내부 순번을 표현합니다.
...
파일에 직접 저장된 객체의 `Origin.Type`은 `FileObjectOrigin`입니다. `Whole`은 객체 전체, `Header`는 `n g obj`, `EndObj`는 `endobj` 범위를 나타냅니다.

Go 모델의 `ObjectStreamOrigin`은 `Container`, `ContainerSpan`, `Index`, `HeaderPair`로 압축 객체의 출처를 표현합니다. `HeaderPair`는 디코딩된 소스 안의 객체 번호·상대 오프셋 쌍 범위입니다.
...
이 형식에는 원본·디코딩 소스의 바이트 본문과 `Reader`를 저장하지 않습니다. 따라서 JSON만으로 해당 바이트를 읽거나 원본 PDF 전체를 재구성할 수는 없습니다.
