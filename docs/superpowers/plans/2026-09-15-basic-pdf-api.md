...
- [x] Write tests using synthetic PDF bytes; assert `Texts[0].Font == Texts[1].Font` for a shared resource and inequality for distinct same-name resources. Assert transformed path coordinates and image matrices match expected values.
...
- [x] Add CLI tests covering a missing path.
...
- `go test ./... -count=1`: both packages passed.
- `go vet ./...`: passed.
