...
**Goal:** Implement the designed Go types and an initial pure-Go reader producing document-wide text, vector graphic, and image placement slices, with original source spans.
...
- Content scope: ordered page traversal, text show operations, ToUnicode mappings, paths, image placements, graphics state and Form traversal. Preserve operations and diagnose unsupported rendering effects.
...
- [x] Run synthetic content and CMap tests.

## Task 4: CLI, documentation, integration and review

Files: cmd/gopd/main.go, README.md, integration_test.go.

- [x] Provide a CLI with page and element counts and extracted text. Add a text output option without serializing source readers or font binaries.
...
- [x] Run `gofmt`, `go test ./...`, `go vet ./...`, bounded fuzzing of the lexer/object parser.
...
- [x] Record verification results and only claim the checks whose final outputs pass.
...
- Final formatting check: clean. Final `go test ./...`: root and CLI packages pass. Final `go vet ./...`: exit 0.
