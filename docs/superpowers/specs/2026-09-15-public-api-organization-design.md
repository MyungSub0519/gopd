...
외부 패키지 테스트로 파일 기본 파싱, reader 상세 파싱, 제한 전달, 객체·스트림 접근, 구문·값·좌표 기능을 검증한다. 기존 합성 PDF 테스트와 `go vet`를 유지한다. 공개 선언의 변경은 문서화와 Deprecated 표시뿐이며 함수·타입을 제거하거나 패키지를 이동하지 않는다. 기능 추가, 새로운 출력 형식, 파싱 최적화는 범위에 포함하지 않는다.
