...
func TestBasicContentOrderAndSharedResources(t *testing.T) {
	...
	if graphic.Page != 0 || !graphic.Stroke || graphic.Fill || graphic.Style.LineWidth != 2 || graphic.Segments[1].Points[0] != (Point{30, 50}) {
...
func TestBasicUserPDF(t *testing.T) {
	if _, err := os.Stat("테스트PDF.pdf"); os.IsNotExist(err) {
		t.Skip("user fixture is not present")
	}
	p, err := ParsePDF("테스트PDF.pdf")
	if err != nil {
		t.Fatal(err)
	}
