...
func (d *Document) readHeaderAndXRefs() error {
	...
	d.Structure.Header = &Header{Span: Span{Source: 1, Start: int64(header), End: int64(header + 8)}, Version: Version{Major: version[0] - '0', Minor: version[2] - '0'}}
	d.markRegion(RegionHeader, d.Structure.Header.Span)
	if header != 0 {
		d.Structure.Diagnostics = append(d.Structure.Diagnostics, Diagnostic{Severity: SeverityWarning, Code: "leading-data", Message: "bytes precede PDF header", Span: Span{Source: 1, End: int64(header)}})
	}
	...
	tail := FileTail{StartXRef: Span{Source: 1, Start: int64(start), End: int64(start + 9)}, Offset: int64(offset), OffsetRaw: Span{Source: 1, Start: int64(raw), End: int64(rawEnd)}, EOFMarker: Span{Source: 1, Start: int64(pos), End: int64(pos + 5)}}
	d.Structure.Tails = append(d.Structure.Tails, tail)
	d.markRegion(RegionFileTail, Span{Source: 1, Start: int64(start), End: int64(pos + 5)})
	...
	if encrypt, e := dict.Get("Encrypt"); e == nil {
		_, null := encrypt.Value.(Null)
		d.Encrypted = !null
	} else if !errors.Is(e, ErrMissingKey) {
		return e
	}
	return nil
}
...
func (d *Document) readXRefChain(offset int64, seen map[int64]bool) error {
	...
	if len(seen) >= d.Options.Limits.MaxXRefSections {
		return errors.New("xref section limit exceeded")
	}
	seen[offset] = true
	...
	section.ID = SectionID(len(d.Structure.XRefs) + 1)
	...
	if section.XRefStm != nil {
		...
		if seen[hybridOff] || len(seen) >= d.Options.Limits.MaxXRefSections {
			return errors.New("hybrid xref cycle or section limit")
		}
		seen[hybridOff] = true
		...
		if hybrid.Form != XRefStream {
			return errors.New("XRefStm does not refer to xref stream")
		}
		hybrid.ID = SectionID(len(d.Structure.XRefs) + 1)
...
func (d *Document) mergeEntries(section XRefSection) error {
	...
	if len(d.entries) > d.Options.Limits.MaxObjects {
		return errors.New("xref object limit exceeded")
	}
	return nil
}
...
func (d *Document) reserveXRefRange(count uint64) error {
	...
	if d.xrefRanges >= limit || count > uint64(limit-d.xrefRecords) {
		return errors.New("cumulative xref record/subsection limit exceeded")
	}
	d.xrefRanges++
...
func (d *Document) readXRefSection(offset int64) (XRefSection, error) {
	if offset < 0 || offset >= int64(len(d.data)) {
		return XRefSection{}, errors.New("xref offset outside file")
	}
	pos := int(offset)
...
func (d *Document) xrefLinks(section *XRefSection) error {
	dict, ok := section.Trailer.Value.(Dictionary)
	if !ok {
...
func (d *Document) readXRefTable(start int) (XRefSection, error) {
	section := XRefSection{Form: XRefTable, Offset: int64(start)}
	pos := start + 4
	total := 0
	for {
		...
		if word == "trailer" {
			obj, n, err := parseObjectWithLimits(d.data[pos:], 1, int64(pos), d.Options.Limits)
			if err != nil {
				return section, err
			}
			pos += n
			section.Trailer = obj
			section.Span = Span{Source: 1, Start: int64(start), End: int64(pos)}
			d.markRegion(RegionXRef, Span{Source: 1, Start: int64(start), End: int64(begin)})
			d.markRegion(RegionTrailer, Span{Source: 1, Start: int64(begin), End: int64(pos)})
		...
		header := Span{Source: 1, Start: int64(begin), End: int64(pos)}
		r := XRefRange{First: uint32(first), Count: uint32(count), Header: &header}
		for i := uint64(0); i < count; i++ {
			...
			record := XRefRecord{Number: uint32(first + i), Span: Span{Source: 1, Start: int64(recordStart), End: int64(pos)}}
			switch flag {
			case "n":
				record.Entry = InUseEntry{Offset: int64(off), Generation: uint16(gen)}
			case "f":
				if off > 1<<32-1 {
					return section, errors.New("free object number overflows uint32")
				}
				record.Entry = FreeEntry{NextFree: uint32(off), Generation: uint16(gen)}
			default:
...
func directInt(dict Dictionary, key Name) (int64, error) {
	o, err := dict.Get(key)
	if err != nil {
		return 0, err
	}
...
func (d *Document) readXRefStream(start int) (XRefSection, error) {
	section := XRefSection{Form: XRefStream, Offset: int64(start)}
	obj, err := d.parseIndirect(start)
	if err != nil {
		return section, err
	}
	stream, ok := obj.Body.Value.(Stream)
	if !ok {
		return section, errors.New("xref target is not a stream")
	}
	kind, err := stream.Dictionary.Get("Type")
	if err != nil || kind.Value != Name("XRef") {
		return section, errors.New("xref stream lacks /Type /XRef")
	}
	section.StreamID = &obj.ID
	section.Span = obj.Origin.(FileObjectOrigin).Whole
	section.Trailer = Object{Span: stream.DictionarySpan, Value: stream.Dictionary}
	...
	for i := 0; i < len(indices); i += 2 {
		...
		if first > 1<<32-1 || count > int64(d.Options.Limits.MaxObjects-total) || count > int64((len(data)-pos)/rowWidth) || count > size || first > size-count {
			return section, errors.New("xref stream range outside size/input/limit")
		}
		if err := d.reserveXRefRange(uint64(count)); err != nil {
			return section, err
		}
		r := XRefRange{First: uint32(first), Count: uint32(count)}
		for j := int64(0); j < count; j++ {
			...
			record := XRefRecord{Number: uint32(first + j), Span: Span{Source: source.ID, Start: int64(begin), End: int64(pos)}}
			switch fields[0] {
			case 0:
				if fields[1] > 1<<32-1 || fields[2] > 65535 {
					return section, errors.New("xref free entry overflow")
				}
				record.Entry = FreeEntry{NextFree: uint32(fields[1]), Generation: uint16(fields[2])}
			case 1:
				if fields[1] > uint64(len(d.data)) || fields[2] > 65535 {
					return section, errors.New("xref in-use entry out of range")
				}
				record.Entry = InUseEntry{Offset: int64(fields[1]), Generation: uint16(fields[2])}
			case 2:
				if fields[1] > 1<<32-1 || fields[2] > 1<<32-1 {
					return section, errors.New("compressed xref entry overflow")
				}
				record.Entry = CompressedEntry{StreamNumber: uint32(fields[1]), Index: uint32(fields[2])}
			default:
				record.Entry = UnknownXRefEntry{Type: fields[0]}
				d.Structure.Diagnostics = append(d.Structure.Diagnostics, Diagnostic{Severity: SeverityWarning, Code: "unknown-xref-entry", Message: fmt.Sprintf("unsupported xref entry type %d", fields[0]), Span: record.Span})
			}
			r.Records = append(r.Records, record)
		}
		total += int(count)
