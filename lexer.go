...
func newSyntaxScanner(data []byte, source SourceID, offset, maxTokenBytes int64) (syntaxScanner, error) {
...
func (s *syntaxScanner) errorAt(pos int, message string) error {
	return &syntaxError{Position: Position{Source: s.source, Offset: s.offset + int64(pos)}, Message: message}
}

func (s *syntaxScanner) token(kind TokenKind, start int) (Token, error) {
	if int64(s.pos-start) > s.maxTokenBytes {
		return Token{}, s.errorAt(start, "token byte limit exceeded")
	}
	return Token{Kind: kind, Span: Span{Source: s.source, Start: s.offset + int64(start), End: s.offset + int64(s.pos)}}, nil
}
...
func (s *syntaxScanner) nextNonTrivia() (Token, error) {
	for {
		token, err := s.next()
		if err != nil || (token.Kind != TokenWhitespace && token.Kind != TokenComment) {
			return token, err
		}
	}
}

func (s *syntaxScanner) next() (Token, error) {
	start := s.pos
	if s.pos == len(s.data) {
		return s.token(TokenEOF, start)
	}
	c := s.data[s.pos]
	if isPDFWhitespace(c) {
		...
		return s.token(TokenWhitespace, start)
	}
	s.pos++
	switch c {
	case '%':
		...
		return s.token(TokenComment, start)
	case '[':
		return s.token(TokenArrayOpen, start)
	case ']':
		return s.token(TokenArrayClose, start)
	case '<':
		if s.pos < len(s.data) && s.data[s.pos] == '<' {
			s.pos++
			return s.token(TokenDictOpen, start)
		}
		for s.pos < len(s.data) {
			...
			if c == '>' {
				return s.token(TokenHexString, start)
			}
			if !isPDFWhitespace(c) && hexNibble(c) < 0 {
				return Token{}, s.errorAt(s.pos-1, "invalid hexadecimal string digit")
			}
		}
		return Token{}, s.errorAt(start, "unterminated hexadecimal string")
	case '>':
		if s.pos < len(s.data) && s.data[s.pos] == '>' {
			s.pos++
			return s.token(TokenDictClose, start)
		}
		return Token{}, s.errorAt(start, "unmatched hexadecimal string terminator")
	case '(':
		depth := 1
		for s.pos < len(s.data) {
			c = s.data[s.pos]
			...
			case '\\':
				if s.pos == len(s.data) {
					return Token{}, s.errorAt(start, "unterminated literal string escape")
				}
			...
			case ')':
				depth--
				if depth == 0 {
					return s.token(TokenLiteralString, start)
				}
			}
		}
		return Token{}, s.errorAt(start, "unterminated literal string")
	case '/':
		for s.pos < len(s.data) && !isPDFWhitespace(s.data[s.pos]) && !isPDFDelimiter(s.data[s.pos]) {
			if s.data[s.pos] == '#' {
				if len(s.data)-s.pos < 3 || hexNibble(s.data[s.pos+1]) < 0 || hexNibble(s.data[s.pos+2]) < 0 {
					return Token{}, s.errorAt(s.pos, "invalid name escape; expected two hexadecimal digits")
				}
				if s.data[s.pos+1] == '0' && s.data[s.pos+2] == '0' {
					return Token{}, s.errorAt(s.pos, "NUL byte is not allowed in a PDF name")
				}
				s.pos += 3
			} else {
				s.pos++
		...
		return s.token(TokenName, start)
	case ')', '{', '}':
		return Token{}, s.errorAt(start, "unexpected delimiter")
	default:
		for s.pos < len(s.data) && !isPDFWhitespace(s.data[s.pos]) && !isPDFDelimiter(s.data[s.pos]) {
			s.pos++
			if err := s.checkSize(start); err != nil {
...
func numberTokenKind(raw []byte) TokenKind {
	i := 0
