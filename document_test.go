...
func TestDocumentResolvesIndirectLengthAndPreservesBinary(t *testing.T) {
	...
	obj, err := d.Resolve(Reference{ID: ObjectID{Number: 3}})
	if err != nil {
		t.Fatal(err)
	}
	s := obj.Value.(Stream)
	if s.Boundary != StreamFromLength || s.Encoded == nil {
...
func TestDocumentDecodesFlateAndCachesSource(t *testing.T) {
	...
	next, err := d.DecodeStream(obj.Value.(Stream))
...
func TestDocumentIncrementalFreeEntryShadowsOlderObject(t *testing.T) {
	...
	if _, err := d.Resolve(Reference{ID: ObjectID{Number: 2}}); err == nil {
...
func TestDocumentRejectsGenerationMismatchAndXRefCycle(t *testing.T) {
	...
	if _, err := d.Resolve(Reference{ID: ObjectID{Number: 1, Generation: 1}}); err == nil {
...
func TestDocumentRejectsInvalidBoundsAndHugeCounts(t *testing.T) {
	...
	for _, span := range []Span{{Source: 1, Start: -1, End: 1}, {Source: 1, Start: 2, End: 1}, {Source: 999, End: 1}, {Source: 1, End: 1 << 62}} {
...
func FuzzDocument(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		d, err := Parse(bytes.NewReader(data), int64(len(data)), ReadOptions{MaxFileBytes: 65536, Limits: Limits{MaxDepth: 32, MaxObjects: 128, MaxXRefSections: 16, MaxDecodedBytes: 65536}})
		if err != nil {
			return
		}
		for number, record := range d.entries {
			...
			case InUseEntry:
				id = ObjectID{Number: number, Generation: e.Generation}
			case CompressedEntry:
				id = ObjectID{Number: number}
			default:
				continue
			}
			obj, e := d.Load(id)
			...
			if stream, ok := obj.Body.Value.(Stream); ok {
