...
func initialContentState() contentState {
	return contentState{graphics: GraphicsState{CTM: IdentityMatrix(), LineWidth: 1, MiterLimit: 10, Stroke: Color{Space: "DeviceGray", Components: []float64{0}}, Fill: Color{Space: "DeviceGray", Components: []float64{0}}, StrokeAlpha: 1, FillAlpha: 1, BlendMode: "Normal", Complete: true}, text: textState{font: -1, hscale: 1}}
}
...
func (c *contentInterpreter) pathOperation(op Operation, index int) error {
	...
	var points []Point
	for i := 0; i < len(n); i += 2 {
		points = append(points, c.state.graphics.CTM.Transform(Point{n[i], n[i+1]}))
	}
	if op.Operator == "re" {
		points = c.rectanglePoints(Rect{Point{n[0], n[1]}, Point{n[0] + n[2], n[1] + n[3]}})
	}
	if op.Operator == "v" {
...
func (c *contentInterpreter) rectanglePoints(r Rect) []Point {
	...
	return []Point{m.Transform(r.Min), m.Transform(Point{r.Max.X, r.Min.Y}), m.Transform(r.Max), m.Transform(Point{r.Min.X, r.Max.Y})}
}
...
func (c *contentInterpreter) showText(op Operation, index int) error {
	...
	for _, element := range elements {
		...
		for _, code := range codes {
			...
			origin := matrix.Transform(Point{0, t.rise})
			end := matrix.Transform(Point{advance, t.rise})
			if !finitePoint(origin) || !finitePoint(end) {
				return fmt.Errorf("text glyph coordinate overflow")
			}
			text.Glyphs = append(text.Glyphs, Glyph{Code: code.bytes, Unicode: code.unicode, Origin: origin, Advance: Point{end.X - origin.X, end.Y - origin.Y}, DecodeComplete: code.complete, WidthKnown: widthKnown})
...
func (c *contentInterpreter) extGState(name Name, op Operation) error {
	...
	for _, entry := range dict.Entries {
		...
		case "LW", "LC", "LJ", "ML":
			...
			if err = c.execute(Operation{Operator: mapped, Operands: []Object{value}, Span: op.Span}, -1); err != nil {
				return err
			}
		case "CA", "ca":
			n, e := Number(value)
		...
		case "RI":
			if err = c.execute(Operation{Operator: "ri", Operands: []Object{value}, Span: op.Span}, -1); err != nil {
				return err
			}
		case "Font":
			array, ok := value.Value.(Array)
