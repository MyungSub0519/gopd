...
// Semantic work is charged for every visit, including cached resource uses.
func (b *semanticBuilder) chargeSemantic(kind string, span Span) error {
	if b.semanticObjects >= b.maxSemanticObjects {
...
func (c *contentInterpreter) interpretSource(source Source) error {
	...
	for i := 0; i < len(tokens); {
		...
		object, consumed, values, e := syntax.ParseObjectWithValueBudget(data[token.Span.Start:], source.ID, token.Span.Start, c.b.doc.Options.Limits, c.b.maxValues-c.b.contentValues)
		if e != nil {
			return e
		}
		c.b.contentValues += values
		if consumed <= 0 {
...
func (c *contentInterpreter) xobject(name Name, op Operation, index int) error {
	...
	if optional {
		c.b.diag("unsupported-content-effect", "XObject optional-content visibility is retained without evaluation", op.Span)
	}
	switch subtype {
	case "Image":
