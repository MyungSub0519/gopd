...
func (d *Document) DecodeStream(stream Stream) (Source, error) {
	if d.Encrypted {
		return Source{}, errors.New("encrypted stream decoding is not supported")
	}
	if _, err := stream.Dictionary.Get("F"); err == nil {
		return Source{}, errors.New("external stream data is not supported")
	} else if !errors.Is(err, ErrMissingKey) {
		return Source{}, err
	}
	if stream.Encoded == nil {
		return Source{}, errors.New("stream boundary is unresolved")
	}
	...
	if remaining <= 0 {
		return Source{}, errors.New("decoded stream byte budget exhausted")
	}
	...
	if object, e := stream.Dictionary.Get("Filter"); e == nil {
		...
		case Array:
			filters = value.Items
		default:
			return Source{}, errors.New("invalid stream /Filter")
		}
	} else if !errors.Is(e, ErrMissingKey) {
		return Source{}, e
	}
	params := make([]*Object, len(filters))
	if object, e := stream.Dictionary.Get("DecodeParms"); e == nil {
		...
		case Dictionary:
			if len(filters) != 1 {
				return Source{}, errors.New("DecodeParms dictionary needs one filter")
			}
			params[0] = &object
		case Array:
			if len(value.Items) != len(filters) {
				return Source{}, errors.New("Filter/DecodeParms array lengths differ")
			}
		...
		default:
			return Source{}, errors.New("invalid /DecodeParms")
		}
	} else if !errors.Is(e, ErrMissingKey) {
		return Source{}, e
	}
	steps := make([]Transform, 0, len(filters))
	for i, object := range filters {
		...
		if !ok {
			return Source{}, errors.New("filter name is not a PDF name")
		}
		data, err = decodeFilter(name, data, params[i], remaining)
		if err != nil {
			return Source{}, fmt.Errorf("/%s: %w", name, err)
		}
		steps = append(steps, Transform{Kind: TransformFilter, Name: name, Params: params[i]})
	}
	if int64(len(data)) > remaining {
		return Source{}, errors.New("decoded stream byte budget exceeded")
	}
	id := SourceID(len(d.Sources) + 1)
	if id == 0 {
		return Source{}, errors.New("source ID limit exceeded")
	}
	source := Source{ID: id, Reader: bytes.NewReader(data), Size: int64(len(data)), Origin: &Derivation{Input: *stream.Encoded, Steps: steps}}
	d.Sources[id] = source
...
func decodeFilter(name Name, data []byte, params *Object, limit int64) ([]byte, error) {
	var result []byte
	var err error
	switch name {
	case "FlateDecode", "Fl":
...
func predictorInt(dict Dictionary, key Name, fallback int64) (int64, error) {
	o, err := dict.Get(key)
...
func applyPredictor(data []byte, params *Object, limit int64) ([]byte, error) {
	if params == nil {
		return data, nil
	}
	if _, ok := params.Value.(Null); ok {
		return data, nil
	}
	dict, ok := params.Value.(Dictionary)
	if !ok {
